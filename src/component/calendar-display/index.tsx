...
const CalendarDisplay = (props: CalendarDisplayProps) => {
    ...
    function getDaysInMonth() {
        return 33 - new Date(props.date.getFullYear(), props.date.getMonth(), 33).getDate();
    }
    ...
    return (
        <div className={clsx(s['calendar-display'], props.isRight ? s['right'] : s['left'])}>
            <div className={clsx(s['week-days-header'])}>
                {weekDays.map((day, index) => (
                    <span
                        key={index}
                        className={clsx(s['week-day'])}
                    >{day}</span>
                ))}
            </div>
